...
中国語・日本語の両方に対応した音声認識で文字起こしし、確認してから送信します。マイク音声は `MediaRecorder` で一発話として録音し、停止後に `/api/stt` へ1回だけ送ります。ブラウザの認識イベントを送信内容に使わないため、認識セッションの再開による文字列の重複は発生しません。音声入力の開始時と回答受信後は入力欄へ自動フォーカスしないため、Androidのソフトキーボードは入力欄を直接タップするまで表示されません。
...
flowchart LR
    subgraph Client["ブラウザ (Web PWA)"]
        ...
        Speech["MediaRecorder<br>(一発話の音声収録)"]
...
Web側では、立ち絵・表情、キャラクタープロフィールと声設定の書き出し、録音を1回だけSTTへ送る音声入力経路、音声コマンド、STT・LLM・TTSの比較・計測を検証しています。
