...
ヘッダーからいつでも HSK 級を切り替えられます。級内の語彙・文法を軸にしつつ、不自然に硬くならないよう平易な言い換えを優先します。趣味分野の固有表現（作品名・料理名など）は、その趣味が現在の話題に直接関係するときに限り、級を超えて自然に使われます。
...
中国語・日本語の両方に対応したブラウザの音声認識で、話している途中から文字を表示し、確認してから送信します。ブラウザが認識を開き直したときに確定済みの文字列を再掲しても、蓄積済みテキストと内容で突き合わせて重なりを落とし、さらに送信前に同じ並びの隣接反復を畳むため、同じ文が2回入りません。音声入力の開始時と回答受信後は入力欄へ自動フォーカスしないため、Androidのソフトキーボードは入力欄を直接タップするまで表示されません。
...
- **キャラクター**: 20体の立ち絵・10表情・声・プロフィールを一覧確認し、ブラウザ上で調整できます。「メイン設定」と、趣味への誘導強度・成人向け話題の許容値・非公開の人物補足を持つ「バックサイド設定」をキャラクターごとに保存できます。プリセットとの差分はコードとして書き出せます。
...
`#dev` → **「LLM 比較」** では、同じ発話を複数の Friend と複数の LLM に送り、キャラクター差とモデル差を一度に比較できます。

1. 比較したい発話を入力し、Friend、話題区分（P0〜P3）、HSK、反復回数を選びます。P4は無効です。
2. 比較するモデルを選びます。初期選択は Cydonia 24B v4.1、Venice Uncensored、Qwen3 Max です。P3ではDeepSeekを選択できません。
3. `Friend数 × モデル数 × 反復回数` とリクエスト数を確認し、**「選択条件で生成」** を押します。1回60件まで、同時実行は3件です。
4. 各試行の中国語、日本語訳、ピンイン、添削、語彙、表情、自動分類、速度、トークン、実費を確認します。必要なら拒否・はぐらかし・婉曲に応答・応答・破損の手動判定を付けます。
5. **「Friendごと」「モデルごと」** で表示順を切り替えます。

入力と回答本文は開いている画面内だけに保持し、IndexedDBの履歴には保存しません。履歴に残るのはモデル・Friend・話題区分・判定・速度・費用・スキーマ検査などのメタデータだけです。比較は実際のOpenRouter APIを呼ぶため、表示される総リクエスト数を確認してください。
...
flowchart LR
    subgraph Client["ブラウザ (Web PWA)"]
        ...
        Speech["Web Speech API (STT)<br>(再掲の重なり吸収つき)"]
...
Web側では、立ち絵・表情、キャラクタープロフィールと声設定の書き出し、音声認識結果の重なり吸収と隣接反復の畳み込み、認識の開き直し、音声コマンド、STT・LLM・TTSの比較・計測を検証しています。
...
すべての変更は [CHANGELOG.md](CHANGELOG.md) に記録しています。最新は **T-110: キャラクターのメイン設定とバックサイド設定を分離** です。
