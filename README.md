...
設定画面は OpenRouter API キーと、自動読み上げ・マイクの言語・無音の待機時間・声調カラーだけです。会話モデルと音声モデルは固定で、画面には出ません。友達の声で変えられるのは「声の高さ」だけで、話者や調整値は開発者モード側で作り込みます。
...
### 5. Fish Audio S2.1 Pro による音声合成 (TTS)
...
すべての変更は [CHANGELOG.md](CHANGELOG.md) に記録しています。最新は **T-104: README・変更履歴・テスター向けガイドを更新** です。
