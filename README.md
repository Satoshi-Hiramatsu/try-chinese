...
### STEP 1: OpenRouter API キーを登録する（任意）

**キーが無くても会話と音声は動きます（無料モード）。** タイトル画面のメニューに「APIキー」があり、`未設定 · 無料モードで動きます` のように状態が出ます。

1. タイトル画面の **「APIキー」**、または会話画面ヘッダーの **キー状態バッジ** を押します。設定画面（「その他」→「設定」）の先頭にも同じ入力欄があります。
2. **OpenRouter API Key**（`sk-or-v1-...`）を入力し、「確認」でキーが有効か調べてから「保存」します。お持ちでない場合は [openrouter.ai/settings/keys](https://openrouter.ai/settings/keys) で作成・チャージしてください。
3. 設定画面で **会話用 AI モデル**（推奨: `DeepSeek V4.1 Flash`）と **音声合成 TTS モデル** を選び、「設定を保存」を押します。

キーの状態は **有効 / 無効 / 残高切れ** の3つで表示します（通常のキーではアカウント残高そのものは取得できません）。会話や音声が残高切れ（402）で返ると、その場で無料モードに切り替えて送り直します。

#### 無料モード（キー未入力・無効・残高切れのとき）

| 経路 | 無料モードでの動き |
| --- | --- |
| 会話（LLM） | Worker の無料 LLM（`nvidia/nemotron-3-super-120b-a12b:free`、混雑時は `nex-agi/nex-n2.5-pro:free`）に固定。設定画面のモデル指定は無視されます |
| 音声（TTS） | 友達の声設定はそのまま、再生時だけ `fish-audio/s2.1-pro-free:free` に差し替え。Fish の話者IDを持たない友達はブラウザ音声になります |
| 音声入力（STT） | ブラウザの音声認識のまま |

無料モードは Worker に置いた **所有者の無料専用キー**（secret `OPENROUTER_API_KEY`）で代行します。Worker は所有者キーでは `:free` モデル以外を通さないため、利用者に有料モデルを使われることはありません。
...
4. 立ち絵横の ⚙️ アイコンから音声モデルと話者を選び、試聴して「保存する」を押すと、その友達専用の声として記録されます。
   - **音声モデルは OpenRouter の音声出力モデルから選べます**。Kokoro と Qwen は8種類の声質キャラクター一覧から、それ以外のモデルはそのモデルが公開する話者一覧から選びます。話者一覧を公開しないモデル（Fish Audio など）は、話者IDの直接入力と揺らぎの調整で声を固定できます。
   - **モデルを切り替えても、前のモデルで設定した話者IDと調整値は残ります**。聴き比べのために往復しても設定をやり直す必要はありません。
   - 作り込んだ有料モデルの声で聞くには有効な API キーと利用料金が必要です。キーが無いあいだは無料版 Fish に差し替えて再生します（設定画面と管理ダッシュボードに注記が出ます）。合成に失敗した場合はブラウザ音声へ自動切り替えせず、エラーを表示します。

#### 声の管理ダッシュボード（全キャラクターの一括設定）

URL の末尾に `#admin` を付けると、**声の管理ダッシュボード** が開きます。設定モーダルの「声の管理ダッシュボードを開く」からも入れます。

- 20人の顔・音声モデル・話者ID・調整値を一覧し、カード上で直接変更できます。
- **話者ID未設定** と **話者ID重複**（同じモデルで同じ話者IDを使っている）をバッジで警告します。
...
`worker/.dev.vars` に開発用の OpenRouter キーを置けます。ここに置いたキーは、ブラウザ側でキーが未入力のときの **無料モードの代行キー** になります（`:free` モデルにしか使われません）。設定しなくても、ブラウザの設定画面からキーを入力すれば動作します。
...
無料モードで使う LLM は `wrangler.jsonc` の `vars.OPENROUTER_FREE_MODELS`（カンマ区切り、先頭から順に試す）で決まります。
...
無料モード（利用者がキーを入れなくても動く状態）にするには、所有者の **無料専用** OpenRouter キーを secret に置きます。Worker はこのキーで `:free` モデル以外を呼びません。
