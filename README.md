...
選択中の TTS モデル（Qwen, Kokoro, Edge Neural 等）に合わせて、友達ごとに「知的な青年男性」「快活な男性」「明るい女性」「やさしい癒やし声」などの話者キャラクターを個別に設定し、ワンタップで試聴できます。
...
- 音声認識（中国語・日本語両対応）により、声で話しかけるだけで文字起こし＆送信が可能。
...
3. お好みの **会話 AI モデル**（推奨: `Gemini 2.5 Flash` または `DeepSeek Chat`）と、**音声合成 TTS モデル**（推奨: `Qwen Audio 3.0 TTS Flash`）を選択して「設定を保存」をクリックします。
   - ※ キーをまだ設定しない場合でも、ブラウザ標準音声（Edge等）で無料でお試しいただけます。
...
3. 友達カードの **「声質」** ボタンから、その友達専用の声のトーンやピッチ・スピードを自由にカスタマイズ・試聴できます。
