  ...
  const startListening = useCallback(() => {
    ...
    recognizer = createSpeechRecognizer({
      ...
      onFinalResult: (finalSpeech) => {
        ...
        // 入力欄に残っていた文と重なる分は落とす。マイクを開き直した直後に
        // 前回の認識結果が先頭に再掲されても、同じ文が2回入らない。
        const next = collapseAdjacentRepeat(mergeTranscript(baseText, parsed.content))
